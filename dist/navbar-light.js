...
(() => {
  ...
  function milliseconds(value, fallback = 0) {
    ...
    if (input.endsWith("s")) return (Number.parseFloat(input) || 0) * 1e3;
  ...
  var NavbarLight = class {
    ...
    configure() {
      const config = new Map(
        [...this.root.querySelectorAll("[data-mwp-config-value]")].map((element) => [element.dataset.mwpConfigValue, element.textContent.trim()])
      );
    ...
    publicApi() {
      ...
      return Object.freeze({
        ...
        get state() {
          return navbar.root.dataset.state;
        }
      });
    }
    isCollapsed() {
  ...
  function initNavbarLight(root = document) {
    return [...root.querySelectorAll(ROOT_SELECTOR)].filter((element) => !element.mwpNavbarLight).map((element) => new NavbarLight(element));
  }
